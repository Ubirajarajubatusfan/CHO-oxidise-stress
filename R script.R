...
hamster <- useEnsembl(  # Подключение к базе данных генов китайского хомячка Ensembl 
  biomart = "genes",
...
gene_up <- deg %>% filter(log2FoldChange > 0) %>% pull(ENTREZID) %>% unique() # извлечение генов с повышенной экспрессией
...
write.csv(as.data.frame(ekegg), "/home/bumblebee/rstudio/counts2/KEGG_results.csv", row.names = FALSE) # таблица с результатами KEGG enrichment

saveRDS(ekegg, "/home/bumblebee/rstudio/counts2/ekegg.rds") # бинарный файл с результатами

######создание графиков#####

if(nrow(ekegg) > 0) {
  png("KEGG_cnetplot.png", width = 1800, height = 1400, res = 200)
  print(cnetplot(ekegg, showCategory = 10))
  dev.off()
}

if(nrow(ekegg) > 0) {
  png("KEGG_dotplot.png", width = 1200, height = 800, res = 150)
  print(dotplot(ekegg, showCategory = 15, title = "KEGG Pathway Enrichment"))
  dev.off()
}

if(nrow(ekegg) > 0) {
  png("KEGG_barplot.png", width = 1200, height = 800, res = 150)
